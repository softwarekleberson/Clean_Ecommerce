CREATE TABLE tb_customer (
    ...
    cpf VARCHAR(15) NOT NULL COMMENT 'Brazilian CPF document number (numbers only)',
    full_name VARCHAR(255) NOT NULL COMMENT 'Customer full name',
    birth_date DATE NOT NULL COMMENT 'Customer birth date',
    password_hash VARCHAR(255) NOT NULL COMMENT 'Hashed customer password',
    gender ENUM('MALE', 'FEMALE', 'NOT_INFORMED') NOT NULL COMMENT 'Customer gender',
    area_code VARCHAR(2) NOT NULL COMMENT 'Phone area code (DDD)',
	...
	CREATE INDEX idx_tb_customer_full_name ON tb_customer (full_name);
